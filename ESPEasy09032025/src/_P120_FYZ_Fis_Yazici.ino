...
void FYZ_click1() {
  ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, "fyzart"}, false);
}
void FYZ_longPressStart1() {
  Settings.UseSerial = true;
  ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, "settings"}, false);
}
void FYZ_longPressStop1() {
  Settings.UseSerial = false;
}
void FYZ_click2() {
  ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, "fyztop"}, false);
  if (FYZ_Kopya)
    ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, "fyzkop"}, false);
}
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        const FormSelectorOptions selector120model(optionCount0, options0);
        selector120model.addFormSelector(F("Model"), F("plugin_120_model"), choice0);
        ...
        selector120prn.addFormSelector(F("Tek Fiş"), F("plugin_120_tek_prn"), choice2);
        ...
        selector120prn.addFormSelector(F("Artı Fiş"), F("plugin_120_art_prn"), choice3);
        ...
        selector120prn.addFormSelector(F("Ek Fiş"), F("plugin_120_ek_prn"), choice5);
        ...
        selector120prn.addFormSelector(F("Toplam Fiş"), F("plugin_120_top_prn"), choice4);
        addButton(options2[ExtraTaskSettings.TaskDevicePrint[2]], F("Fiş Dizayn Menüsüne Git"));
#if FEATURE_SD
        byte choice6 = ExtraTaskSettings.TaskDevicePrint[3];
        selector120prn.addFormSelector(F("SD data"), F("plugin_120_sd_prn"), choice6);
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        ...
        ExtraTaskSettings.TaskDevicePrint[4] = getFormItemInt(F("plugin_120_sd_prn"));
        ...
        options2[ExtraTaskSettings.TaskDevicePrint[3]].toCharArray(Settings.urun_ek_prn,  options2[ExtraTaskSettings.TaskDevicePrint[3]].length() + 1);
        options2[ExtraTaskSettings.TaskDevicePrint[4]].toCharArray(Settings.sd_prn,  options2[ExtraTaskSettings.TaskDevicePrint[4]].length() + 1);
        sum_f = 0;
        i = 0;
        success = true;
        break;
    ...
    case PLUGIN_FIFTY_PER_SECOND:
      {
        ...
        if (String(webapinettartim_son,int(ExtraTaskSettings.TaskDeviceValueDecimals[0])) == String(webapinettartim,int(ExtraTaskSettings.TaskDeviceValueDecimals[0])))
          fyzurun_stabil_sayisi++;
        else
          fyzurun_stabil_sayisi = 0;
        if (fyzurun_stabil_sayisi >= FYZ_Gecikme) {
          XML_STABIL_S = "ST";
          if ((webapinettartim_son > FYZ_Hedef) || (topla_i == 2) || ((topla_i == 1) && (FYZ_Mod != 2))) {
            if (hayvan_modu == 0) {
              String fyz_data = "";
              if (topla_i == 1)
                fyz_data += "fyzurunart&";
              ...
              fyz_data += String(sec_URUN_buton);
              fyz_data += "&0&0";
              fyz_data += "&" + String(sec_M_buton[0]);
              fyz_data += "&" + String(sec_M_buton[1]);
              fyz_data += "&" + String(sec_M_buton[2]);
              fyz_data += "&" + String(sec_M_buton[3]);
              fyz_data += "&" + String(sec_M_buton[4]);
              fyz_data += "&" + String(sec_M_buton[5]);
              fyz_data += "&" + String(sec_M_buton[6]);
              fyz_data += "&" + String(sec_M_buton[7]);
              ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, fyz_data.c_str()}, false);
              hayvan_modu = 2;
            }
          }
        }
          ...
          XML_STABIL_S = "US";
          if (((webapinettartim_son < FYZ_Hedef) && (hayvan_modu == 2)) && (FYZ_Mod == 2)) {
            hayvan_modu = 0;
            topla_i = 1;
          }
        }
          ...
          if (inChar == ExtraTaskSettings.TaskDeviceSonByte) {
            ...
            if ((FYZ_Mod == 3) || (FYZ_Mod == 4))
              ...
              if ((FYZ_Mod == 1) && (webapinettartim > 0.0001)) {
                ...
                komut = XML_NET_S;
                ExecuteCommand_all({EventValueSource::Enum::VALUE_SOURCE_WEB_FRONTEND, komut.c_str()}, false);
              }
              if (FYZ_Mod == 2) {
                if (i >= FYZ_Sayac) {
                  i--;
            ...
            if ((FYZ_Mod == 3) || (FYZ_Mod == 4))
              ...
              //if (FYZ_Mod == 2) {
                if (i >= FYZ_Sayac) {
                  ...
                  sum_f_d[i] = webapinettartim;
                  sum_f = sum_f + sum_f_d[i];
                  webapinettartim_son = sum_f / float(i + 1);
                  i++;
                }
              //}
            }
