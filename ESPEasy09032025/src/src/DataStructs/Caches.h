...
struct ExtraTaskSettings_cache_t {
  ...
  uint8_t      bas[VARS_PER_TASK] = { 0 };
  uint8_t      bit[VARS_PER_TASK] = { 0 };
...
struct Caches {
  ...
  uint8_t getTaskDeviceValueBas(taskIndex_t TaskIndex,
                                 uint8_t     rel_index);

  uint8_t getTaskDeviceValueBit(taskIndex_t TaskIndex,
                                  uint8_t     rel_index); 
