...
uint8_t Caches::getTaskDeviceValueBas(taskIndex_t TaskIndex, uint8_t rel_index)
{
  if (validTaskIndex(TaskIndex) && (rel_index < VARS_PER_TASK)) {
    auto it = getExtraTaskSettings(TaskIndex);

    if (it != extraTaskSettings_cache.end()) {
      return it->second.bas[rel_index];
    }
  }
  return 0;
}

uint8_t Caches::getTaskDeviceValueBit(taskIndex_t TaskIndex, uint8_t rel_index)
{
  if (validTaskIndex(TaskIndex) && (rel_index < VARS_PER_TASK)) {
    auto it = getExtraTaskSettings(TaskIndex);

    if (it != extraTaskSettings_cache.end()) {
      return it->second.bit[rel_index];
    }
  }
  return 0;
}
...
void Caches::updateExtraTaskSettingsCache()
{
  ...
  if (validTaskIndex(TaskIndex)) {
    ...
    for (size_t i = 0; i < VARS_PER_TASK; ++i) {
      ...
      tmp.bas[i] = ExtraTaskSettings.TaskDeviceValueBas[i];
      tmp.bit[i] = ExtraTaskSettings.TaskDeviceValueBit[i];
      #if FEATURE_PLUGIN_STATS
