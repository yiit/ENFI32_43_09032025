...
void task_button(int page, int taskIndex) {
  ...
  if (validPluginID_fullcheck(static_cast<pluginID_t>(Settings.TaskDeviceNumber[taskIndex]))) {
  ...
  else
  {
    ...
    switch (Settings.TaskDeviceNumber[taskIndex]) {
      ...
      case 120:
        ...
        break;
      case 121:
        if (!SecuritySettings.LisansControlFYZURUN) {
      ...
      case 130:
        ...
        break;
      case 131:
        if (!SecuritySettings.LisansControlEYZURUN) {
    ...
    addFormCheckBox(F("APP Aktif"), F("TDE"),
...
void devicePage_show_task_values(taskIndex_t taskIndex, deviceIndex_t DeviceIndex)
{
  ...
  if (!device.Custom && (valueCount > 0))
  {
    ...
    if (device.FormulaOption)
    {
      ...
      html_table_header(F("Başlangıç"), 10);
...
# endif // if FEATURE_PLUGIN_STATS
    ...
    for (uint8_t varNr = 0; varNr < valueCount; varNr++)
    {
      ...
      if (device.FormulaOption)
      {
        ...
        html_TD();
        const String idba = getPluginCustomArgName(F("TDVBA"), varNr);  // ="taskdevicevaluebaşlangıç"
        addNumericBox(idba,  Cache.getTaskDeviceValueBas(taskIndex, varNr), 0, 1024);

        html_TD();
        const String idbi = getPluginCustomArgName(F("TDVBI"), varNr);  // ="taskdevicevaluebitiş"
        addNumericBox(idbi, Cache.getTaskDeviceValueBit(taskIndex, varNr), 0, 1024);
      }
      ...
      if (device.configurableDecimals())
      {
        ...
        const __FlashStringHelper *decimal[] = {
          F("0"),
          F("0.0"),
          F("0.00"),
          F("0.000"),
        ...
        constexpr int decimalCount = NR_ELEMENTS(decimal);
        const FormSelectorOptions selector(decimalCount, decimal);
