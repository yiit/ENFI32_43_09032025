...
#  endif // ifdef WEBSERVER_RULES_DEBUG
      ...
      else
      {
        ...
        addHtml("/rules/" + String(fi.Name) + "'>");
        //addHtml(F(".txt'>"));
        addHtml(String(fi.Name) + "</a></TD>");
        //addHtml(F(".txt</a></TD>"));
...
bool handle_rules_edit(String originalUri, bool isAddNew) {
  ...
  if (isAddNew || (originalUri.startsWith(F("/rules/")))) {
    ...
    else
    {
      ...
      fileName = originalUri;
      eventName = FileNameToEvent(fileName);
    }

        // Güvenlik önlemi: Üst dizine çıkmayı engelle
        fileName.replace("..", "");
        fileName.replace("//", "/");

            // Dizin oluşturma (varsa alt dizinleri oluşturur)
    String dirPath = fileName.substring(0, fileName.lastIndexOf('/'));
    if (!LittleFS.exists(dirPath)) {
      if (!LittleFS.mkdir(dirPath)) {
        addLog(LOG_LEVEL_ERROR, concat(F("Failed to create directory: "), dirPath));
        return false;
      }
    }
    ...
    addFormTextBox(F("Event name")            // Label
                   ...
                   , isAddNew                 // required
                   , F("[A-Za-z0-9_/.-]+")        // validation pattern
                   );
    addHelpButton(F("Tutorial_Rules"));
