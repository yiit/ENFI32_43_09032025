...
struct tm ESPEasy_time::addDays(const struct tm& source, int days) const {
  struct tm temp = source;
  time_t t = mktime(&temp);
  if (t == -1) return temp;
  t += days * 86400; // 1 gün = 86400 saniye
  localtime_r(&t, &temp);
  return temp;
}
...
/********************************************************************************************\
   Date/Time string formatters
 \*********************************************************************************************/
String ESPEasy_time::getDateStringPlusDays(int days, char dateDelimiter) const {
  struct tm future_tm = addDays(local_tm, days);
  return formatDateString(future_tm, dateDelimiter);
}
